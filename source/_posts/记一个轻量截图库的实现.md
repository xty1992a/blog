...
```
代码示例：
 {% codepen xty1992a gOOjoqK dark result 450 %}
...
而window同时受容器和model的限制，准确说，受两者重叠部分限制，不能超出重叠部分构成的矩形。它的限制器在它的外部。
当model移动时，以x为例，最小值为 window.x-(model.width-window.width),最大值为window.x
当model缩放时，即便被限制，也应保持比例，因此需要比较window与model的长宽比（均比较width/height的比值），长或宽只能限制一个。
...
具体到事件上。只需要在原鼠标事件回调中，检查`touches`是否存在且是否有两个touchPoint存在。
